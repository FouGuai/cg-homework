...
Color Texture::sample(const SampleParams &sp) {
  int max_level = static_cast<int>(mipmap.size()) - 1;
  // TODO: Task 6: Fill this in.
  if (sp.lsm == L_ZERO) {
    return sp.psm == P_NEAREST ? sample_nearest(sp.p_uv, 0)
                               : sample_bilinear(sp.p_uv, 0);
  } else if (sp.lsm == L_NEAREST) {
    int nearest_level = static_cast<int>(std::round(get_level(sp)));
    nearest_level = std::min(nearest_level, max_level);
    return sp.psm == P_NEAREST ? sample_nearest(sp.p_uv, nearest_level)
                               : sample_bilinear(sp.p_uv, nearest_level);
  } else {
    float level = get_level(sp);
    int level0 = static_cast<int>(std::floor(level));
    int level1 = static_cast<int>(level + 1);
    level0 = std::min(level0, max_level);
    level1 = std::min(level1, max_level);
    ...
    float s0 = level - level0;
    return c1 * s0 + c0 * (1 - s0);
  }
...
float Texture::get_level(const SampleParams &sp) {
  ...
  float L = std::sqrt(std::max(dot(dx, dx), dot(dy, dy)));
  float level = std::log2(L);
  level = std::clamp(level, 0.0f, float(mipmap.size() - 1));
  return level;
}
